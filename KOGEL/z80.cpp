...
//MYSTUFF
static unsigned int trapthis=0;
static unsigned int trapped=0;
static unsigned int trapaddr=0xffff;  //assumes PC never gets here
static int input_pending=0;
static dword ip_port;
static byte ip_byt;
static byte ip_byt_orig;
static byte ip_shift=0;
static unsigned char blank_mono=0;
static unsigned int num_cycs=0;
static int Z80_IRQ=0;          // Current IRQ status. Checked after EI occurs 

//this is to cut the autorepeat down to size
static unsigned int vars_done=0;	//tests whether 6233/4   (5) has been written to - and we can update it with my own reprate

//static unsigned long cyc;
static int donmi=0;
static int doP80=0;
static int P80bit=5;
static int startirq=0;

unsigned int cycl_d=0;//noof cycles so far this execution of z80
unsigned int tapecyc=0; //measures pulse widths output by port84

//extern int trace_on;
//extern int trace_handle;
unsigned int ei_addr=0;


extern byte z80ports_in[];
extern void update_sound();
extern void flush_sound();
static byte tbyt;
...
extern  int  _declspec(dllexport) start_trace()
{
	int ret;
		trace_on=1;
   /* Open for write */
...
extern  int  _declspec(dllexport) stop_trace()
{
		trace_on=0;
		fclose(stream2);
return(1);			
...
unsigned int Z80_Execute ()
{
	...
	static unsigned ret;
	static z80_icount_last=Z80_IPeriod;
...
//		Z80_ICount=240;
	...
	//	cyc++; //used for the SOUND stuff
		...
		//tapecyc+=(z80_icount_last-Z80_ICount);
...
*/
		...
		Z80_ICount-=num_cycs;
		tapecyc+=num_cycs;
//		if(tapecyc>128)//good output
//		if(tapecyc>40)//got Filename at 0
		if(tapecyc>tape_spd+tape_spd_adjust)
			update_tape();//		Periodic ?22050 Hz (original time) update
...
extern  void  _stdcall set_pc (unsigned int huj)
{
	R.PC.D=huj & 0xffff;
}
...
extern  unsigned int    _stdcall set_trapthis (unsigned int huj)
{
	trapthis=huj;
	return(trapthis);
...
extern  unsigned int    _stdcall set_trapaddr (unsigned int huj)
{
	trapaddr=huj;
	return(trapaddr);
}
...
extern  void    _stdcall s_step(unsigned int huj)
{
	int tmp;
	tmp=Z80_IPeriod;
	Z80_IPeriod=1;
...
extern  unsigned int  _stdcall get_tape_icount()
{
	return(tapecyc);
}
...
extern  void  _stdcall set_regs (Z80_Regs i)
{
R.AF=i.AF; 	
R.BC=i.BC; 	
R.DE=i.DE; 	
...
extern  void  _stdcall get_regs (Z80_Regs *i)
{
//	char lbl[100];
