...
void vid_latch96k(UBYTE value)
{
  ...
  //      update_vid_maps96k();
        ...
        if (((value & 0x40)==0x40) && Line_Blank==0)  
        {       
...
//      if((port & 0xff7f)==0xff7f)port=0x7f;
	...
	// IMPORTANT this must be 207f for pengo and FTEXT interline writes to work
	// I suspect that some bank latch writes dont result in changing the written bank
	// i.e some xx7f writes other than 207f occur which result in a write to bank 1 rather
	// than a video write.
